# Laboratorio de riesgo cuantitativo de commodities

Material de apoyo para el punto 4 del proyecto: análisis cuantitativo de los riesgos
de mercados de commodities — metales (oro, plata, platino, cobre), energéticos
(petróleo, gas natural) y granos (trigo, maíz). Tres herramientas que calculan
**exactamente lo mismo** por caminos distintos, para que el equipo pueda verificar
sus propios números.
...
| Archivo | Qué es |
|---|---|
| `index.html` | Laboratorio interactivo de 10 pasos. Se abre en cualquier navegador, sin instalación ni servidor. |
...
1. **Explorar con la serie demo.** Abrir `index.html` y recorrer los diez pasos.
...
2. **Descargar datos propios.** Yahoo Finance (`GC=F`, `SI=F`, `PL=F`, `HG=F`,
   `CL=F`, `NG=F`, `ZW=F`, `ZC=F`, `^GSPC`, `DX-Y.NYB`), LBMA, CME o FRED.
   Mínimo tres años de historia diaria.
...
3. En GitHub, pestaña **Actions** del repo, corre manualmente
   *"Actualizar datos de commodities"* una vez (botón *Run workflow*) para generar
...
`fetch_datos.py` descarga diez tickers:

| Activo | Ticker | Categoría |
|---|---|---|
| Oro | `GC=F` | Metal precioso |
| Plata | `SI=F` | Metal precioso |
| Platino | `PL=F` | Metal precioso |
| Cobre | `HG=F` | Metal industrial |
| Petróleo (WTI) | `CL=F` | Energético |
| Gas natural | `NG=F` | Energético |
| Trigo | `ZW=F` | Grano |
| Maíz | `ZC=F` | Grano |
| S&P 500 | `^GSPC` | Benchmark |
| Dólar (DXY) | `DX-Y.NYB` | Benchmark |

Para agregar otro metal, energético, grano u otro instrumento, añade una
...
Si quieres que el nuevo activo tenga símbolo y color propios en el laboratorio
(en vez del color genérico de reserva), añade también una entrada en el
objeto `META` dentro de `index.html`.

Nota: la serie **DEMO** incrustada en `index.html` (para explorar sin conexión)
sigue teniendo sólo los seis activos originales — oro, plata, platino, cobre,
S&P 500 y dólar. Los cuatro nuevos activos (petróleo, gas natural, trigo, maíz)
sólo aparecen cuando el sitio carga **datos en vivo** desde `precios.json`.
...
| Justificación de métricas | Por qué esa métrica y no otra, para este activo y este horizonte |
...
| Simulación Montecarlo | Rango proyectado con supuestos explícitos, comparado contra el VaR analítico |
| Conclusión accionable | Ranking de activos con criterio explícito de decisión |

## Simulación Montecarlo (paso 9)

Proyecta miles de trayectorias futuras del precio a partir de las propiedades
estadísticas ya medidas en los pasos 2–6. Tres métodos, seleccionables en el
laboratorio y replicables en `analisis_metales.py`:

- **Bootstrap histórico** (por omisión) — remuestrea con reemplazo los
  rendimientos observados. Conserva la forma exacta de la distribución
  —asimetría, colas gruesas— sin asumir ninguna forma paramétrica.
- **t-Student calibrada** — grados de libertad estimados a partir de la
  curtosis muestral, para simular colas gruesas de forma paramétrica.
- **Normal (browniano geométrico)** — el caso clásico de libro de texto; sirve
  como referencia de cuánto cambia el resultado al asumir normalidad.

El laboratorio muestra un abanico de trayectorias (bandas de 50% y 90%), el
histograma del precio proyectado al final del horizonte, los percentiles
P5–P95, y una comparación directa contra el VaR analítico de la sección 5
escalado por la raíz del tiempo.

**Limitación que hay que declarar en el reporte:** los tres métodos asumen
rendimientos independientes entre periodos (i.i.d.). Ninguno reproduce el
agrupamiento de volatilidad — que una racha turbulenta aumente la probabilidad
de que el siguiente periodo también lo sea, como sí captura un EWMA o un
GARCH — ni tendencias o reversión a la media. Es la misma limitación que ya
aparece en el escalamiento por raíz del tiempo de la sección 5; el Montecarlo
no la resuelve, solo la hace más visible al compararla contra la distribución
empírica completa.

Verificación cruzada:

```bash
python analisis_metales.py --csv datos_demo.csv --principal Oro \
       --mc-metodo boot --mc-horizonte 60 --mc-sims 5000 --mc-semilla 42 \
       --json metricas.json
```

Con semilla fija el resultado es reproducible; el laboratorio HTML no fija
semilla a propósito, para que cada corrida muestre la variabilidad propia del
muestreo aleatorio — el botón "Repetir simulación" lo hace visible sin
necesidad de tocar ningún otro parámetro.
...
**Contraste de frecuencia.** Correr el mismo activo en diaria y en mensual. El VaR
